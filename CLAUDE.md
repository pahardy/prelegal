...
The Mutual NDA is the only document currently implemented. AI chat is not yet implemented.
...
- **Mutual NDA creator** (`/nda`): the existing form + preview flow, unchanged in functionality.
- **Navbar**: shared component with the PreLegal brand and a sign-out button.
- **Backend** (`backend/`): FastAPI uv project. Starts an SQLite DB at `$DB_PATH` (default `/data/prelegal.db`) with a `users` table. Serves the static frontend. Health check at `/health`.
- **Docker**: multi-stage `Dockerfile` builds the frontend then runs the backend. `docker-compose.yml` mounts a named volume for the DB.
